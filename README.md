...
- On launch, requests Google Drive/Sheets access.
- Creates an `XCODCalculator` folder in Google Drive.
- Creates or reuses a Google Sheet named `delivery boy`; the delivery-boy picker reads from its `DeliveryBoys` tab.
...
1. Authorizes the user for Google Drive/Sheets access when the app launches.
2. Creates an `XCODCalculator` folder in Google Drive.
3. Creates or reuses a Google Sheet file named `delivery boy`.
4. Reads delivery boys from the `DeliveryBoys` tab with `id` and `name` columns.
5. Appends every deposit to the `CodEntries` tab with date, delivery-boy, total, cash, online, and due columns.
6. Reads entries back to show each delivery boy's total due amount.
